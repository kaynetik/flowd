...
pub fn topological_tip_cherry_picks(
    plan: &Plan,
) -> Result<Vec<CherryPick>, IntegrationRefusal> {
...
mod tests {
    use super::*;
    use crate::orchestration::{Plan, PlanStep, PlanStatus, StepStatus};
    ...
    fn step_branch_ref_uses_simple_uuid_and_sanitises_segments() {
        ...
        assert_eq!(r, "flowd/My-Project/00000000000000000000000000000000/step-with-spaces");
    }
    ...
    fn refuses_draft_plan() {
        ...
        assert!(matches!(
            err,
            IntegrationRefusal::PlanStatus { observed: PlanStatus::Draft }
        ));
    }
    ...
    fn refuses_running_plan() {
        ...
        assert!(matches!(
            err,
            IntegrationRefusal::PlanStatus { observed: PlanStatus::Running }
        ));
    }
    ...
    fn refuses_failed_plan() {
        ...
        assert!(matches!(
            err,
            IntegrationRefusal::PlanStatus { observed: PlanStatus::Failed }
        ));
    }
    ...
    fn refuses_cancelled_plan() {
        ...
        assert!(matches!(
            err,
            IntegrationRefusal::PlanStatus { observed: PlanStatus::Cancelled }
        ));
    }
    ...
    fn refuses_interrupted_plan() {
        ...
        assert!(matches!(
            err,
            IntegrationRefusal::PlanStatus { observed: PlanStatus::Interrupted }
        ));
    }
    ...
    fn linear_chain_picks_only_final_tip() {
        ...
        let ids: Vec<&str> = result.cherry_picks.iter().map(|p| p.step_id.as_str()).collect();
        assert_eq!(ids, vec!["c"]);
    }
